...
By using trigonometric identities combined with integration by substitution/parts we'd like to evaluate integrals of the form $\displaystyle \int \sin^{j}{(x)} cos^{k}{(x)} dx$ (for integer values of $j$ and $k$), and $\displaystyle \int \tan^{k}{(x)} \sec^{j}{(x)} dx$ (for integer values of $k$ and $j$).

**Goals:** Work together to discover the techniques that work for these anti-derivatives.
...
5. In general, how would you go about trying to and $\displaystyle \int \sin^{j}{(x)} \cos^{k}{(x)} dx$, where $j$ is odd? (Hint: consider the previous three problems.)
...
10. Further investigations you may consider (especially for mathematics, physics, and engineering majors): We also would like to be able to solve integrals of the form $\displaystyle \int \tan^{k}{(x)} \sec^{j}{(x)} dx$. These two functions play well with each other, since the derivative of $\tan{(x)}$ is $\sec^{2}{(x)}$ and the derivative of $\sec{(x)}$ is $\sec{(x)} \tan{(x)}$, and since there is a pythagorean identity relating them. It sometimes works to use $u = \tan{(x)}$ and it sometimes works to use $u = \sec{(x)}$. Based on the values of $k$ and $j$, which substitution should you use? Are there cases for which neither substitution works?
